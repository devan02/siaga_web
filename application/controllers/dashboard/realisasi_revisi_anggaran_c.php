...
class Realisasi_revisi_anggaran_c extends CI_Controller {
	...
	function index(){
		...
		$key = "";
		$data = array(
			  'page' 		=> "dashboard/realisasi_revisi_anggaran_v",
			  'induk_menu' 	=> "realisasi_revisi_anggaran",
			  'menu' 		=> "realisasi_revisi_anggaran",
			  'title' 		=> "REALISASI REVISI RKAP",
			  'url'			=> base_url().'dashboard/realisasi_revisi_anggaran_c/simpan', 
			  'url_del'		=> base_url().'dashboard/realisasi_revisi_anggaran_c/hapus',
			  'departemen'	=> $this->dep_div->departemen(),
			  'koper'		=> $this->koper->get_koper_all($key),
			  'no_surat'	=> $this->biaya_luar->get_no_surat($key),
	...
	function data_anggaran(){
		...
		$data = $this->model->get_realisasi($tahun,$bagian,$sub_bagian,$kode_perkiraan,$kriteria,$keyword);
		echo json_encode($data);
	}

	function get_realisasi_dpbm(){
	...
	function do_realisasi(){
		...
		$data['data'] = $this->model->do_realisasi($id_anggaran,$periode);
		...
		if($sudah_realisasi > 0){
			...
			$data['keterangan'] = "BELUM DIREALISASI";
		}
		echo json_encode($data);
	}
	...
	function simpan(){
		...
		if($jenis_realisasi == "DPBM"){
			...
			$this->master_model_m->save_log('Simpan', 'Melakukan penyimpanan data pada MENU', 'REALISASI REVISI ANGGARAN', $kegiatan2, $objek);
	...
	function hapus(){
		...
		// redirect('dashboard/realisasi_revisi_anggaran_c');
	...
	function simpan_anggaran_tambahan(){
		...
		$id_jenis_anggaran = addslashes($this->input->post('id_jenis_anggaran'));
		...
		$this->biaya_luar->update_status($id_surat);

		// save_log($jenis, $kegiatan, $modul, $kegiatan2, $objek)
		$kegiatan2 = "dengan Kode Anggaran ".$kode_anggaran." dan NO SURAT ".$id_jenis_anggaran;
		$objek = $kode_anggaran." - ".$id_jenis_anggaran;
		$this->master_model_m->save_log('Simpan', 'Melakukan penyimpanan data pada MENU', 'REALISASI REVISI ANGGARAN INPUT ANGGARAN TAMBAHAN', $kegiatan2, $objek);
