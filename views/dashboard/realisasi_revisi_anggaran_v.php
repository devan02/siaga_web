...
$(document).keyup(function(e) {
    if (e.keyCode == 27){
        ...
        $('#search_koper').val("");
        $('#popup_dpbm').fadeOut('slow');
        $('#search_dpbm').val("");
        $('#popup_rab').fadeOut('slow');
        $('#search_rab').val("");
        $('#popup_no_surat').fadeOut('slow');
        $('#search_surat').val("");
    }
});
...
$(document).ready(function(){
    ...
    $('#simpan').click(function(){
        ...
        $('#popup_load').show();
        $.ajax({
            url : '<?php echo base_url(); ?>dashboard/realisasi_revisi_anggaran_c/simpan',
    ...
    $('#tambah_baru').click(function(){
        if($('#kode_perkiraan').val() == ""){
            $('#radioExample10').prop('checked',true);
            $('#radioExample9').removeAttr('checked');
            $('.view_per_koper').show();
            $('#empty_koper').show();
            $('#empty_koper').delay(5000).fadeOut('slow');
            $('body,html').animate({
    ...
    $('#simpan_tambahan').click(function(){
        ...
        $('#popup_load').show();
        $.ajax({
            url : '<?php echo base_url(); ?>dashboard/realisasi_revisi_anggaran_c/simpan_anggaran_tambahan',
            data : $('#form_tambahan').serialize(),
            type : "POST",
            dataType : "json",
            async : false,
            success : function(result){
    ...
    $('#ya_hapus').click(function(){
        ...
        $('#popup_load').show();
        $.ajax({
            url : '<?php echo base_url(); ?>dashboard/realisasi_revisi_anggaran_c/hapus',
...
function get_anggaran(){
    ...
    ajax = $.ajax({
        url : '<?php echo base_url(); ?>dashboard/realisasi_revisi_anggaran_c/data_anggaran',
        data : {
            tahun:tahun,
            bagian:bagian,
        ...
        success : function(result){
            ...
            for(var i=0; i<result.length; i++){
                var total = 0;
                var jumlah = result[i].JUMLAH;
                var realisasi = parseFloat(result[i].REALISASI);

                if(result[i].JENIS_ANGGARAN == "Barang"){
                    total = parseFloat(result[i].TOTAL);  
                } else {
                    total = parseFloat(result[i].TOTAL_PELAKSANAAN);
                }

                var rkap = parseFloat(jumlah) * parseFloat(result[i].HARGA);
                var sisa = rkap - realisasi;

                if(realisasi < rkap && realisasi > 0){
                    warna = "kuning";
                    color = "";
                }else if(realisasi > rkap && realisasi > 0){
                    warna = "merah";
                    color = "color:#fff;";
                }else if(result[i].STS_TAMBAHAN == 1 && result[i].STS_REVISI == 6){
                    warna = "biru";
                    color = "color:#fff;";
                }else if(result[i].STS_TAMBAHAN == 4){
                    warna = "orange";
                    color = "color:#fff;";
                }else if(result[i].STS_REVISI == 5){
                    warna = "biru";
                    color = "color:#fff;";
                }else if(result[i].STS_TAMBAHAN == 3){
                    warna = "ungu";
                    color = "color:#fff;";
                }else{
                    warna = "putih";
                ...
                $isi += "<tr style='cursor:pointer; "+color+"' class='"+warna+"' onclick=do_realisasi("+result[i].ID_ANGGARAN+","+result[i].STS_INPUT+");>"+
                            ...
                            "<td style='white-space:nowrap;'>Rp. "+NumberToMoney(result[i].HARGA)+"</td>"+
                            "<td align='center'>"+result[i].REAL_VOL+"</td>"+
                            "<td style='white-space:nowrap;'>Rp. "+NumberToMoney(rkap)+"</td>"+
                            "<td style='white-space:nowrap;'>Rp. "+NumberToMoney(realisasi)+"</td>"+
                            "<td style='white-space:nowrap;'>"+isPositive(sisa)+"</td>"+
                        "</tr>";
            }
            $('#data tbody').html($isi);
...
function do_realisasi(id_anggaran,periode){
    ...
    $.ajax({
        url : '<?php echo base_url(); ?>dashboard/realisasi_revisi_anggaran_c/do_realisasi',
...
function ajax_data_dpbm(){
    ...
    ajaxe = $.ajax({
        url : '<?php echo base_url(); ?>dashboard/realisasi_revisi_anggaran_c/cari_dpbm',
        type : "GET",
        dataType : "json",
        data : {
            keyword : keyword,
...
function get_realisasi_dpbm(id_anggaran){
    $.ajax({
        url : '<?php echo base_url(); ?>dashboard/realisasi_revisi_anggaran_c/get_realisasi_dpbm',
        ...
        success : function(result){
            ...
            for(var i=0; i<result.length; i++){
                $isi += "<tr style='cursor:pointer;' onclick=get_dpbm_click("+result[i].ID_DPBM+");>"+
                            "<td align='center'>"+result[i].NO_DPBM+'.'+result[i].ID_DPBM+"</td>"+
...
function get_dpbm_click(id_dpbm){
    ...
    $('#popup_load').show();
    $.ajax({
        url : '<?php echo base_url(); ?>dashboard/realisasi_revisi_anggaran_c/get_realisasi_by_id_dpbm',
        data : {id_dpbm:id_dpbm},
        type : "POST",
        dataType : "json",
        async : false,
...
function get_realisasi_rab(id_anggaran){
    $.ajax({
        url : '<?php echo base_url(); ?>dashboard/realisasi_revisi_anggaran_c/get_realisasi_rab',
        data : {id_anggaran:id_anggaran},
        type : "POST",
        dataType : "json",
        async : false,
        success : function(result){
            ...
            for(var i=0; i<result.length; i++){
                ...
                $isi += "<tr style='cursor:pointer;' onclick=get_rab_click("+result[i].ID+"); >"+
                            ...
                            "<td align='center'>"+result[i].NO_KEU+"</td>"+
                            "<td align='center' style='white-space:nowrap;'>"+result[i].NO_SPK+"</td>"+
...
function get_rab_click(id_rab){
    ...
    $('#popup_load').show();
    $.ajax({
        url : "<?php echo base_url(); ?>dashboard/realisasi_revisi_anggaran_c/get_rab_click",
        data : {id_rab:id_rab},
        type : "POST",
        dataType : "json",
        async : false,
        success : function(row){
            ...
            $('#id_hapus').val(row['ID']);
            $('#id_realisasi').val(row['ID_DET_RAB']);

            $('#id_rab').val(row['ID']);
            $('#id_det_rab').val(row['ID_RAB']);
            $('#id_spk').val(row['ID_SPK']);
...
function ajax_no_spm(){
    ...
    ajaxe = $.ajax({
        url : '<?php echo base_url(); ?>dashboard/realisasi_revisi_anggaran_c/popup_data_spm',
        type : "GET",
        dataType : "json",
        data : {
            keyword : keyword,
...
function get_spm_id(id_spm){
    $.ajax({
        url : '<?php echo base_url(); ?>dashboard/realisasi_revisi_anggaran_c/get_spm_id',
        data : {id_spm:id_spm},
        type : "POST",
        dataType : "json",
        async : false,
...
function get_realisasi_spm(id_anggaran){
    $.ajax({
        url : '<?php echo base_url(); ?>dashboard/realisasi_revisi_anggaran_c/get_realisasi_spm',
        data : {id_anggaran:id_anggaran},
        type : "POST",
        dataType : "json",
        async : false,
...
<div class="panel" id="view_realisasi">
    ...
    <form class="form-horizontal" role="form" action="" method="post" id="form_realisasi">
        <div class="panel-body">
                        ...
                        <table class="table table-bordered" id="data">
                            <thead>
                                <tr class="primary">
                                    ...
                                    <th style="vertical-align: middle; text-align:center; white-space:nowrap;">Anggaran (Vol)</th>
                                    <th style="vertical-align: middle; text-align:center; white-space:nowrap;">Harga Satuan (Rp)</th>
                                    <th style="vertical-align: middle; text-align:center; white-space:nowrap;">Realisasi (Vol)</th>
                                    <th style="vertical-align: middle; text-align:center; white-space:nowrap;">Total RKAP</th>
                                    <th style="vertical-align: middle; text-align:center; white-space:nowrap;">Realisasi (Rp)</th>
                                    <th style="vertical-align: middle; text-align:center; white-space:nowrap;">Sisa RKAP (Rp)</th>
                                </tr>
                            </thead>
                            <tbody>
            ...
            <div class="row">
                    ...
                    <div class="panel">
                        ...
                        <div class="panel-body pn">
                            <table class="table table-bordered" id="tes_dpbm">
                                <thead>
                                    <tr class="warning">
                                        <th style="vertical-align: middle; text-align:center;">No DPBM</th>
                                        <th style="vertical-align: middle; text-align:center;">Tanggal</th>
                                        <th style="vertical-align: middle; text-align:center;">Jumlah</th>
                                        <th style="vertical-align: middle; text-align:center;">No KEU</th>
                ...
                <div class="col-sm-7" id="tabel_rab">
                    <div class="panel">
                        <div class="panel-heading">
                            <span class="panel-title">
                                <span class="glyphicons glyphicons-table"></span><span id="judul_tabel_rab">RAB</span>
                            </span>
                        </div>
                        <div class="panel-body pn">
                            <div class="scroll-x">
...
<div class="cd-popup" role="alert">
    <div class="cd-popup-container">
        ...
        <p id="ket_hapus"></p>
        <ul class="cd-buttons">            
            <li><a id="ya_hapus" href="javascript:;">Ya</a></li>
            <li><a id="tidak_hapus" href="javascript:;">Tidak</a></li>
        </ul>
...
<div class="panel" id="view_anggaran_tambahan">
    ...
    <form class="form-horizontal" role="form" action="" method="post" id="form_tambahan">
                ...
                <table class="table table-hover table-bordered" id="data_surat">
                    ...
                    ?>
                        <tr style="cursor:pointer;" onclick="get_surat_id('<?php echo $value_surat->ID; ?>');">
                            <td align="center"><?php echo $no; ?></td>
                            <td align="center"><?php echo $value_surat->NO_SURAT; ?></td>
                            <td><?php echo $value_surat->PROGRAM_BIAYA; ?></td>
                        </tr>
                    <?php
                        }
                    ?>
